        ...
        void Start()
        {
            // Basic: Download from google.
            {
                ObservableWWW.Get("http://google.co.jp/")
                    .Subscribe(
                        x => debug(x.Substring(0, 100)), // onSuccess
                        ex => debugex(ex)); // onError
            }
            ...
            // download after google, start bing download
            {
                ...
                var cancel = query.Subscribe(x => debug(x.google.Substring(0, 100) + ":" + x.bing.Substring(0, 100)));
            ...
            // (It's like Observable.Zip but specialized for single async operations like Task.WhenAll of .NET 4)
            {
                ...
                {
                    debug(xs[0].Substring(0, 100)); // google
                    debug(xs[1].Substring(0, 100)); // bing
                    debug(xs[2].Substring(0, 100)); // unity
                });
            }
            ...
            // with Progress
            {
                ...
                progressNotifier.Subscribe(x => debug(x)); // write www.progress
